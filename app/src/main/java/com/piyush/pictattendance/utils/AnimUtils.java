...
public class AnimUtils {
    ...
    public static void scale(long delay, ViewGroup view) {
        for (int i2 = 0; i2 < view.getChildCount(); i2++) {
            View view1 = view.getChildAt(i2);
            view1.setAlpha(0.0f);
            view1.animate().alpha(1.0f).setDuration(150).setStartDelay(delay).start();
        }
    }
        public static void descale ( long delay, ViewGroup view){
            for (int i2 = 0; i2 < view.getChildCount(); i2++) {
                View view1 = view.getChildAt(i2);
                view1.setAlpha(1.0f);
                view1.animate().alpha(0.0f).setDuration(10).start();
            }
        }
