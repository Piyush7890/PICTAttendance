...
public class ExpansionLayout extends NestedScrollView {
        ...
        if (animated) {
            ...
            valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                ...
                public void onAnimationUpdate(ValueAnimator valueAnimator) {
                   ...
                   // getChildAt(0).setAlpha(1-valueAnimator.getAnimatedFraction());
            ...
            getChildAt(0).setAlpha(1f);
            getChildAt(0).animate().alpha(0f).setDuration(75).start();
    ...
    public void expand(boolean animated) {
        ...
        int expandDuration = (int) (getChildAt(0).getHeight() / getContext().getResources().getDisplayMetrics().density);
        int alphaDuration = ((int) (expandDuration * 2f / 3f));
        if (!isEnabled() || expanded) {
            return;
        }
        ...
        if (animated) {
            ...
            getChildAt(0).setAlpha(0f);
            getChildAt(0).animate().alpha(1f).setStartDelay(expandDuration-75).setDuration(75).start();
            valueAnimator.addListener(new AnimatorListenerAdapter() {
                @Override
                public void onAnimationEnd(Animator animation) {
                    super.onAnimationEnd(animation);
